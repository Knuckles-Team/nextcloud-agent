[ ![Logo](https://docs.nextcloud.com/server/14/developer_manual/_static/logo-white.png) ](https://docs.nextcloud.com/server/14/developer_manual/index.html)
  ...
  * [](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html)
    * [Setup](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#setup)
    * [Routes & controllers](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#routes-controllers)
    * [Database](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#database)
    * [Connect database & controllers](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#connect-database-controllers)
    * [Making things reusable and decoupling controllers from the database](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#making-things-reusable-and-decoupling-controllers-from-the-database)
    * [](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#writing-a-test-for-the-controller-recommended)
      * [Unit tests](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#unit-tests)
    * [Integration tests](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#integration-tests)
    * [Adding a RESTful API (optional)](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#adding-a-restful-api-optional)
    * [Adding JavaScript and CSS](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#adding-javascript-and-css)
    * [Creating a navigation](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#creating-a-navigation)
    * [Creating the content](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#creating-the-content)
    * [Wiring it up](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#wiring-it-up)
    * [Apply finishing touches](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#apply-finishing-touches)
...
[Nextcloud 14 Developer Manual](https://docs.nextcloud.com/server/14/developer_manual/index.html)
  ...
  * [App development](https://docs.nextcloud.com/server/14/developer_manual/app/index.html) »
  * Tutorial
  * [ Edit on GitHub](https://github.com/nextcloud/documentation/edit/stable14/developer_manual/app/tutorial.rst)


* * *
# Tutorial[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#tutorial "Permalink to this headline")
This tutorial will outline how to create a very simple notes app. The finished app is available on [GitHub](https://github.com/nextcloud/app-tutorial#tutorial).
## Setup[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#setup "Permalink to this headline")
First the [development environment](https://docs.nextcloud.com/server/14/developer_manual/general/devenv.html) needs to be set up. This can be done by either [downloading the zip from the website](https://nextcloud.com/install/) or cloning it directly from GitHub:
```
...
Note
`$BRANCH` is the desired Nextcloud branch (e.g. `stable9` for Nextcloud 9, `stable10` for Nextcloud 10, ..., `master` for the upcoming release)
...
Afterwards a skeleton app can be created in the [app store](https://apps.nextcloud.com/developer/apps/generate).
Download the compressed file that contains the generated app and extract it into your `apps/` directory. Afterwards the application can be enabled on the [apps page](http://localhost:8080/index.php/settings/apps).
The first basic app is now available at `http://localhost:8080/index.php/apps/yourappid/`
## Routes & controllers[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#routes-controllers "Permalink to this headline")
...
On the server side we need to register a callback that is executed once the request comes in. The callback itself will be a method on a [controller](https://docs.nextcloud.com/server/14/developer_manual/app/controllers.html) and the controller will be connected to the URL with a [route](https://docs.nextcloud.com/server/14/developer_manual/app/routes.html). The controller and route for the page are already set up in **ownnotes/appinfo/routes.php** :
```
<?php
return ['routes' => [
...
This route calls the controller **OCA\OwnNotes\PageController- >index()** method which is defined in **ownnotes/lib/Controller/PageController.php**. The controller returns a [template](https://docs.nextcloud.com/server/14/developer_manual/app/templates.html), in this case **ownnotes/templates/main.php** :
Note
@NoAdminRequired and @NoCSRFRequired in the comments above the method turn off security checks, see [Controllers](https://docs.nextcloud.com/server/14/developer_manual/app/controllers.html)
```
<?php
...
## Database[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#database "Permalink to this headline")
Now that the routes are set up and connected the notes should be saved in the database. To do that first create a [database schema](https://docs.nextcloud.com/server/14/developer_manual/app/schema.html) by creating **ownnotes/appinfo/database.xml** :
```
<database>
    <name>*dbname*</name>
    <create>true</create>
...
To create the tables in the database, the [version tag](https://docs.nextcloud.com/server/14/developer_manual/app/info.html) in **ownnotes/appinfo/info.xml** needs to be increased:
```
<?xml version="1.0"?>
<info>
    <id>ownnotes</id>
...
Now that the tables are created we want to map the database result to a PHP object to be able to control data. First create an [entity](https://docs.nextcloud.com/server/14/developer_manual/app/database.html) in **ownnotes/lib/Db/Note.php** :
```
<?php
...
Entities are returned from so called [Mappers](https://docs.nextcloud.com/server/14/developer_manual/app/database.html). Let’s create one in **ownnotes/lib/Db/NoteMapper.php** and add a **find** and **findAll** method:
```
<?php
namespace OCA\OwnNotes\Db;
...
## Connect database & controllers[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#connect-database-controllers "Permalink to this headline")
The mapper which provides the database access is finished and can be passed into the controller.
You can pass in the mapper by adding it as a type hinted parameter. Nextcloud will figure out how to [assemble them by itself](https://docs.nextcloud.com/server/14/developer_manual/app/container.html). Additionally we want to know the userId of the currently logged in user. Simply add a **$UserId** parameter to the constructor (case sensitive!). To do that open **ownnotes/lib/Controller/NoteController.php** and change it to the following:
```
<?php
 namespace OCA\OwnNotes\Controller;
...
## Making things reusable and decoupling controllers from the database[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#making-things-reusable-and-decoupling-controllers-from-the-database "Permalink to this headline")
...
Remember how we had all those ugly try catches that where checking for **DoesNotExistException** and simply returned a 404 response? Let’s also put this into a reusable class. In our case we chose a [trait](http://php.net/manual/en/language.oop5.traits.php) so we can inherit methods without having to add it to our inheritance hierarchy. This will be important later on when you’ve got controllers that inherit from the **ApiController** class instead.
...
## Writing a test for the controller (recommended)[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#writing-a-test-for-the-controller-recommended "Permalink to this headline")
...
### Unit tests[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#unit-tests "Permalink to this headline")
...
Because Nextcloud uses [Dependency Injection](https://docs.nextcloud.com/server/14/developer_manual/app/container.html) to assemble your app, it is very easy to write unit tests by passing mocks into the constructor. A simple test for the update method can be added by adding this to **ownnotes/tests/Unit/Controller/NoteControllerTest.php** :
```
<?php
...
If [PHPUnit is installed](https://phpunit.de/) we can run the tests inside **ownnotes/** with the following command:
```
phpunit
...
## Integration tests[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#integration-tests "Permalink to this headline")
...
## Adding a RESTful API (optional)[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#adding-a-restful-api-optional "Permalink to this headline")
A [RESTful API](https://docs.nextcloud.com/server/14/developer_manual/app/api.html) allows other apps such as Android or iPhone apps to access and change your notes. Since syncing is a big core component of Nextcloud it is a good idea to add (and document!) your own RESTful API.
Because we put our logic into the **NoteService** class it is very easy to reuse it. The only pieces that need to be changed are the annotations which disable the CSRF check (not needed for a REST call usually) and add support for [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/Access_control_CORS) so your API can be accessed from other webapps.
...
## Adding JavaScript and CSS[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#adding-javascript-and-css "Permalink to this headline")
To create a modern webapp you need to write [JavaScript](https://docs.nextcloud.com/server/14/developer_manual/app/js.html). You can use any JavaScript framework but for this tutorial we want to keep it as simple as possible and therefore only include the templating library [handlebarsjs](http://handlebarsjs.com/). [Download the file](http://builds.handlebarsjs.com.s3.amazonaws.com/handlebars-v2.0.0.js) into **ownnotes/js/handlebars.js** and include it at the very top of **ownnotes/templates/main.php** before the other scripts and styles:
```
<?php
...
## Creating a navigation[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#creating-a-navigation "Permalink to this headline")
The template file **ownnotes/templates/part.navigation.php** contains the navigation. Nextcloud defines many handy [CSS styles](https://docs.nextcloud.com/server/14/developer_manual/app/css.html) which we are going to reuse to style the navigation. Adjust the file to contain only the following code:
Note
**$l- >t()** is used to make your strings [translatable](https://docs.nextcloud.com/server/14/developer_manual/app/l10n.html) and **p()** is used [to print escaped HTML](https://docs.nextcloud.com/server/14/developer_manual/app/templates.html)
```
<!-- translation strings -->
...
## Creating the content[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#creating-the-content "Permalink to this headline")
...
## Wiring it up[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#wiring-it-up "Permalink to this headline")
...
## Apply finishing touches[¶](https://docs.nextcloud.com/server/14/developer_manual/app/tutorial.html#apply-finishing-touches "Permalink to this headline")
Now the only thing left is to style the textarea in a nicer fashion. To do that open **ownnotes/css/style.css** and replace the content with the following [CSS](https://docs.nextcloud.com/server/14/developer_manual/app/css.html) code:
```
#app-content-wrapper {
    height: 100%;
}
...
[Next ](https://docs.nextcloud.com/server/14/developer_manual/app/startapp.html "Create an app") [](https://docs.nextcloud.com/server/14/developer_manual/app/changelog.html "Changelog")
* * *
