# nextcloud-agent — Concept Overview
...
```json
{
  "mcpServers": {
    "nextcloud-agent": {
      "command": "uv",
      "args": ["run", "--with", "nextcloud-agent", "nextcloud-mcp"],
      "env": {}
    }
  }
}
...
```bash
nextcloud-mcp --transport streamable-http --port 8001
```
