...
`nextcloud-agent` can run as a local stdio process or container, or behind a remote
network boundary. The
[Deployment guide](https://knuckles-team.github.io/nextcloud-agent/deployment/) carries
...
services:
  nextcloud-agent-mcp:
    image: example/nextcloud-agent:mcp
...
|-----------|--------------|----------|------------|
| `example/nextcloud-agent:mcp` | `--target mcp` | `nextcloud-agent[mcp]` — **connector-focused**, includes `epistemic-graph[full]`; no model-orchestration stack | `nextcloud-mcp` |
| `example/nextcloud-agent@sha256:<digest>` | `--target agent` (default) | `nextcloud-agent[agent]` — **agent runtime**, model orchestration + `epistemic-graph[full]` | `nextcloud-agent` |

```bash
docker build --target mcp   -t example/nextcloud-agent:mcp    docker/   # connector-focused MCP server
docker build --target agent -t example/nextcloud-agent:agent-local docker/   # agent runtime
```
...
| Immutable container | deploy `registry.example.invalid/nextcloud-agent@sha256:<digest>` through the operator-selected orchestrator |
